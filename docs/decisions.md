...
## ADR-033: On-Demand Manual Database Backup, Synchronous Prisma WAL Consolidation, and Bypass Deduplication
- **Date:** 2026-09-12
- **Context:** (1) File uploads and database mutations committed to SQLite Write-Ahead Log (`dev.db-wal`) by active server processes (`@prisma/adapter-better-sqlite3`) were not immediately observable to ad-hoc backup connections, causing SHA-256 hash deduplication (`currentHash === latestHash`) to suppress backup snapshot creation even though fresh changes existed in the WAL. (2) Contributors requested a visible, 1-click on-demand manual backup button in the sidebar providing immediate user feedback (loading spinner, success timestamp/filename, or error state) to give peace of mind before closing the app or after large data imports.
- **Decision:**
  1. **Prisma Connection WAL Checkpoint Consolidation (`POST /api/system/backup`):** Created a dedicated system backup API endpoint that runs `prisma.$queryRawUnsafe('PRAGMA wal_checkpoint(TRUNCATE);')` directly through the active server connection before taking backup snapshots, ensuring all in-flight WAL frames are committed to `dev.db`.
  2. **Forced Backup Creation Support (`src/lib/dbBackup.ts`):** Extended `createDbBackup(maxRetained, targetDbPath, force = false)` with `force` parameter. When `force === true`, SHA-256 duplicate hash suppression is bypassed, guaranteeing that an explicit manual request always creates a new timestamped `.db.gz` file.
  3. **1-Click Sidebar Backup Button (`Sidebar.tsx`):** Added `sidebar-backup-btn` directly above the Quit App button in the sidebar footer with responsive collapsed/expanded layouts, Lucide icons (`Database`, `Loader2`, `Check`), and 3.5-second transient success/error feedback with localized tooltips. The button disables itself while loading to prevent rapid duplicate double-writes.
  4. **Automated Verification:** Added unit tests `UT-API-SYSTEM-BACKUP-01` (`system_backup.test.ts`), `UT-UI-SIDEBAR-BACKUP-01`, and `UT-UI-SIDEBAR-BACKUP-02` (`Sidebar.test.tsx`). Registered DOM selector `sidebar-backup-btn` in `docs/tests/SELECTORS.md` and test IDs in `docs/tests/CATALOG.md`. All 61 test files (401 tests) pass with 0 errors.
- **Impact:** Eliminates WAL synchronization lag for backups, empowers users with instant 1-click database durability confirmation, and ensures 100% test and safety compliance.
